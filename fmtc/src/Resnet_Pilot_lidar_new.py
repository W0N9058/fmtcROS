...
class SimpleResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.relu1(out)
        out = self.conv2(out)
        return self.relu2(out) + x # ReLU?äî ?ûÖ?†•?ùÑ ?çî?ïú ?õÑ ?òê?äî ?ù¥?†Ñ?óê ?†Å?ö©?ï† ?àò ?ûà?äµ?ãà?ã§.


class ImageRegressionBase(nn.Module):
    def training_step(self, batch):
        images, labels = batch
        labels = labels.unsqueeze(1).float()  # ?†à?ù¥Î∏îÏùÑ (batch_size, 1) ?òï?ÉúÎ°? Î≥??ôò?ïòÍ≥? Float ????ûÖ?úºÎ°? Î≥?Í≤?
        out = self(images)                  # ?òàÏ∏? ?Éù?Ñ±
        loss = F.mse_loss(out, labels)      # ?Üê?ã§ Í≥ÑÏÇ∞ (?öåÍ∑?Î•? ?úÑ?ïú ?èâÍ∑? ?†úÍ≥? ?ò§Ï∞?)
        return loss
    ...
    def validation_step(self, batch):
        ...
        labels = labels.unsqueeze(1).float()  # ?†à?ù¥Î∏îÏùÑ (batch_size, 1) ?òï?ÉúÎ°? Î≥??ôò?ïòÍ≥? Float ????ûÖ?úºÎ°? Î≥?Í≤?
        out = self(images)                   # ?òàÏ∏? ?Éù?Ñ±
        loss = F.mse_loss(out, labels)       # ?Üê?ã§ Í≥ÑÏÇ∞
        return {"val_loss": loss.detach()}

    def validation_epoch_end(self, outputs):
        batch_losses = [x["val_loss"] for x in outputs]
        epoch_loss = torch.stack(batch_losses).mean()       # ?Üê?ã§ Í≤∞Ìï©
        return {"val_loss": epoch_loss} # ?Üê?ã§ Í≤∞Ìï©
...
class ResNet9(ImageRegressionBase):
    def __init__(self, in_channels):
        ...
        self.classifier = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), # ?îºÏ≤? Îß? ?Å¨Í∏∞Î?? 1x1Î°? Ï°∞Ï†ï
            nn.Flatten(),
            nn.Linear(512, 1)  # ?ïò?Çò?ùò ?ã§?àòÍ∞íÏùÑ ?òàÏ∏?
        )

    def forward(self, xb): # xb?äî Î°úÎìú?êú Î∞∞Ïπò
        out = self.conv1(xb)
        out = self.conv2(out)
...
class Resnet_Pilot:
    ...
    def traffic_light_callback(self, msg):
        ...
        if self.current_stage == 'waiting_stop' and msg.data == 'stop':
            ...
            self.forward = 0.0  # ?äπ?†ï Í∞íÏúºÎ°? ?Ñ§?†ï?ïò?ó¨ ÏßÅÏßÑ
    ...
    def move_to_first_lane(self):
        self.timed_publish(1.0, 2.0)  # ?ôºÏ™ΩÏúºÎ°? 2Ï¥àÍ∞Ñ Ï°∞Ìñ•
        self.timed_publish(-1.0, 2.0)  # ?ò§Î•∏Ï™Ω?úºÎ°? 2Ï¥àÍ∞Ñ Ï°∞Ìñ•
        self.timed_publish(0.0, 3.0)  # ÏßÅÏßÑ 3Ï¥?
        self.timed_publish(-1.0, 2.0)  # ?ò§Î•∏Ï™Ω?úºÎ°? 2Ï¥àÍ∞Ñ Ï°∞Ìñ•
        self.timed_publish(1.0, 2.0)  # ?ôºÏ™ΩÏúºÎ°? 2Ï¥àÍ∞Ñ Ï°∞Ìñ•
        self.timed_publish(0.0, 0.1)  # ÏßÅÏßÑ 0.1Ï¥?
