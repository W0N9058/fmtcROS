...
class ObstacleDetection:
    ...
    def check_obstacle(self):
        ...
        #obstacle_detected = any((-1.8 <= x <= -1.2 and -1<y<=1 and  num >= 20) for (x, y), num in zip(self.cluster_coordinates, self.cluster_point_nums))

        if len(self.cluster_coordinates) == len(self.cluster_point_nums):
            obstacle_detected = any((-1.8 <= x <= -1.2 and -0.6< y <= 0.6 and num >= 20) for (x, y), num in zip(self.cluster_coordinates, self.cluster_point_nums))
        else:
        ...
        if obstacle_detected:
            ...
            if self.count >= 10:
                ...
                indices = [index for index, value in enumerate(self.cluster_coordinates) if (-1.8 <= value[0] <= -1.2 and -1 < value[1] <= 1 and self.cluster_point_nums[index] >= 20)]
